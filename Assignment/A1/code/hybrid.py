...
def my_imfilter(image, kernel, padding_type):
    ...
    padding_type 0: zero padding
                 ...
                 2: symetric
    '''
    # check whether it's grayscale image
    if len(image.shape) == 3:
        gray_image = 0
    else:
        gray_image = 1
    ...
    for i in range(img_row):
        for j in range(img_col):
            if gray_image == 1:
                ...
                convolution_sum = [0.0, 0.0, 0.0]
            # calculating convolution
            for k in range(-a, a+1):
                for t in range(-b, b+1):
                    convolution_sum += new_img[i+a-k][j+b-t]*kernel[a+k][b+t]
                    
            # update the pixel value after filtering
            filtered_img[i][j] = convolution_sum
...
if __name__ == '__main__':
    ...
    """
    # print "img1"
    low_frequencies = my_imfilter(image1, kernel1, 0)
    ...
    """
    # print "img2"
    low_frequencies_temp = my_imfilter(image2, kernel2, 0)
